...
class TestResults(Document):
	def validate(self):
		...
		self.set_dual_analysis_details()

	def set_dual_analysis_details(self):
	...
	def update_calculated_fields(self):
		self.dga_tcgc = round((self.dga_h2 + self.dga_co + self.dga_ch4 + self.dga_c2h6 + self.dga_c2h4 + self.dga_c2h2 + self.dga_c3h8 + self.dga_c3h6), 4)
...
def get_duvals_fault(region):
	...
	if (region):
		if (region == "PD"):
			fault = "<p>PARTIAL DISCHARGE - <strong>PD</strong></p>"
		elif (region == "D1"):
			fault = "<p>DISCHARGE OF LOW ENERGY - <strong>D1</strong></p>"
		elif (region == "D2"):
			fault = "<p>DISCHARGE OF HIGH ENERGY - <strong>D2</strong></p>"
		elif (region == "DT"):
			fault = "<p>THERMAL AND ELECTRICAL FAULT - <strong>DT</strong></p>"
		elif (region == "T1"):
			fault = "<p>THERMAL FAULT AT TEMPERATURE &lt; 300 <sup>O</sup>C - <strong>T1</strong></p>"
		elif (region == "T2"):
			fault = "<p>THERMAL FAULT AT TEMPERATURE 300-700 <sup>O</sup>C - <strong>T2</strong></p>"
		elif (region == "T3"):
			fault = "<p>THERMAL FAULT AT TEMPERATURE &gt; 700 <sup>O</sup>C - <strong>T3</strong></p>"
		elif (region == "NA"):
